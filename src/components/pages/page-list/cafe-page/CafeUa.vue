      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Назва</span>
            <input v-model="cafeUa.name" placeholder="Назва" type="text" />
          </div>
        </div>
        <div class="card-page__description">
          <span>Опис</span>
          <textarea v-model="cafeUa.description" placeholder="Опис"></textarea>
        </div>
        ...
        <div class="card-page__seo">
          <div class="card-page__seo-block">
            ...
            <div class="card-page__seo-input">
              <p>
                URL:
                <input v-model="cafeUa.seoUrl" type="text" placeholder="URL" />
              </p>
              <p>
                Title:
                <input
                  v-model="cafeUa.seoTitle"
                  type="text"
                  placeholder="Title"
                />
              </p>
...
export default {
  ...
  computed: {
    imageSrc() {
      if (this.cafeUa.mainImg.localUrl) {
        return this.cafeUa.mainImg.localUrl;
      }
      if (this.cafeUa.mainImg.imgUrl) {
        return this.cafeUa.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.cafeUa.mainImg.img = "";
        this.cafeUa.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.cafeUa.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.cafeUa.mainImg.id) {
          this.cafeUa.mainImg.id = new Date().valueOf();
        }
        this.cafeUa.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.cafeUa.mainImg.url = "";
      this.cafeUa.mainImg.mainLocal = "";
      this.cafeUa.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
