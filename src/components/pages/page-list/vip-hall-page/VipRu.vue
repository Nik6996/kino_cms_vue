      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Название</span>
            <input v-model="vipRu.name" placeholder="Название" type="text" />
          </div>
        </div>
        <div class="card-page__description">
          <span>Описание</span>
          <textarea
            v-model="vipRu.description"
        ...
        <div class="card-page__seo">
          <div class="card-page__seo-block">
            ...
            <div class="card-page__seo-input">
              <p>
                URL:
                <input v-model="vipRu.seoUrl" type="text" placeholder="URL" />
              </p>
              <p>
                Title:
                <input
                  v-model="vipRu.seoTitle"
                  type="text"
                  placeholder="Title"
                />
              </p>
...
export default {
  ...
  computed: {
    imageSrc() {
      if (this.vipRu.mainImg.localUrl) {
        return this.vipRu.mainImg.localUrl;
      }
      if (this.vipRu.mainImg.imgUrl) {
        return this.vipRu.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.vipRu.mainImg.img = "";
        this.vipRu.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.vipRu.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.vipRu.mainImg.id) {
          this.vipRu.mainImg.id = new Date().valueOf();
        }
        this.vipRu.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.vipRu.mainImg.url = "";
      this.vipRu.mainImg.mainLocal = "";
      this.vipRu.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
