      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Назва</span>
            <input v-model="vipUa.name" placeholder="Назва" type="text" />
          </div>
        </div>
        <div class="card-page__description">
          <span>Опис</span>
          <textarea v-model="vipUa.description" placeholder="Опис"></textarea>
        </div>
        ...
        <div class="card-page__seo">
          <div class="card-page__seo-block">
            ...
            <div class="card-page__seo-input">
              <p>
                URL:
                <input v-model="vipUa.seoUrl" type="text" placeholder="URL" />
              </p>
              <p>
                Title:
                <input
                  v-model="vipUa.seoTitle"
                  type="text"
                  placeholder="Title"
                />
              </p>
...
export default {
  ...
  computed: {
    imageSrc() {
      if (this.vipUa.mainImg.localUrl) {
        return this.vipUa.mainImg.localUrl;
      }
      if (this.vipUa.mainImg.imgUrl) {
        return this.vipUa.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.vipUa.mainImg.img = "";
        this.vipUa.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.vipUa.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.vipUa.mainImg.id) {
          this.vipUa.mainImg.id = new Date().valueOf();
        }
        this.vipUa.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.vipUa.mainImg.url = "";
      this.vipUa.mainImg.mainLocal = "";
      this.vipUa.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
