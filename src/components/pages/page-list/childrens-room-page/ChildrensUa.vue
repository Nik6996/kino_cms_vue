      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Назва</span>
            <input v-model="childrensUa.name" placeholder="Назва" type="text" />
          </div>
        </div>
        <div class="card-page__description">
          <span>Опис</span>
          <textarea
            v-model="childrensUa.description"
            placeholder="Опис"
          ></textarea>
        </div>
          ...
          <div class="card-page__seo-block">
            ...
            <div class="card-page__seo-input">
              ...
              <p class="keywords">
                Keywords:
                <input
                  v-model="childrensUa.seoKeywords"
                  type="text"
                  placeholder="word"
                />
              </p>
              <p class="card-page__seo-description">
                Description:<textarea
                  v-model="childrensUa.seoDescription"
...
export default {
  data() {
    return {
      ...
      //   mainLocal: "",
      // },
      gallary: this.childrensUa.gallary,
    };
  },
  components: {
    GallaryImg,
  ...
  computed: {
    imageSrc() {
      if (this.childrensUa.mainImg.localUrl) {
        return this.childrensUa.mainImg.localUrl;
      }
      if (this.childrensUa.mainImg.imgUrl) {
        return this.childrensUa.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  watch: {
    childrensUa: {
      handler(childrensUa) {
        this.gallary = childrensUa.gallary;
      },
    },
  },
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.childrensUa.mainImg.img = "";
        this.childrensUa.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.childrensUa.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.childrensUa.mainImg.id) {
          this.childrensUa.mainImg.id = new Date().valueOf();
        }
        this.childrensUa.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.childrensUa.mainImg.url = "";
      this.childrensUa.mainImg.mainLocal = "";
      this.childrensUa.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
