      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Назва</span>
            <input v-model="customUa.name" placeholder="Назва" type="text" />
          </div>
        </div>
        <div class="card-page__description">
          <span>Опис</span>
          <textarea
            v-model="customUa.description"
            placeholder="Опис"
          ></textarea>
        </div>
            ...
            <div class="card-page__seo-input">
              ...
              <p class="card-page__seo-description">
                Description:<textarea
                  v-model="customUa.seoDescription"
...
export default {
  ...
  computed: {
    imageSrc() {
      if (this.customUa.mainImg.localUrl) {
        return this.customUa.mainImg.localUrl;
      }
      if (this.customUa.mainImg.imgUrl) {
        return this.customUa.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.customUa.mainImg.img = "";
        this.customUa.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.customUa.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.customUa.mainImg.id) {
          this.customUa.mainImg.id = new Date().valueOf();
        }
        this.customUa.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.customUa.mainImg.url = "";
      this.customUa.mainImg.mainLocal = "";
      this.customUa.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
