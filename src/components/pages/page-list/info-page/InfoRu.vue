      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Название</span>
            <input v-model="infoRu.name" placeholder="Название" type="text" />
          </div>
        </div>
        ...
        <div class="card-page__seo">
          <div class="card-page__seo-block">
            ...
            <div class="card-page__seo-input">
              <p>
                URL:
                <input v-model="infoRu.seoUrl" type="text" placeholder="URL" />
              </p>
              <p>
                Title:
                <input
                  v-model="infoRu.seoTitle"
                  type="text"
                  placeholder="Title"
                />
              </p>
...
export default {
  ...
  computed: {
    imageSrc() {
      if (this.infoRu.mainImg.localUrl) {
        return this.infoRu.mainImg.localUrl;
      }
      if (this.infoRu.mainImg.imgUrl) {
        return this.infoRu.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.infoRu.mainImg.img = "";
        this.infoRu.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.infoRu.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.infoRu.mainImg.id) {
          this.infoRu.mainImg.id = new Date().valueOf();
        }
        this.infoRu.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.infoRu.mainImg.url = "";
      this.infoRu.mainImg.mainLocal = "";
      this.infoRu.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
