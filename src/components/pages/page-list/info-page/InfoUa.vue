      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Назва</span>
            <input v-model="infoUa.name" placeholder="Назва" type="text" />
          </div>
        </div>
        <div class="card-page__description">
          <span>Опис</span>
          <textarea v-model="infoUa.description" placeholder="Опис"></textarea>
        </div>
        ...
        <div class="card-page__seo">
          <div class="card-page__seo-block">
            ...
            <div class="card-page__seo-input">
              <p>
                URL:
                <input v-model="infoUa.seoUrl" type="text" placeholder="URL" />
              </p>
              <p>
                Title:
                <input
                  v-model="infoUa.seoTitle"
                  type="text"
                  placeholder="Title"
                />
              </p>
...
export default {
  ...
  computed: {
    imageSrc() {
      if (this.infoUa.mainImg.localUrl) {
        return this.infoUa.mainImg.localUrl;
      }
      if (this.infoUa.mainImg.imgUrl) {
        return this.infoUa.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.infoUa.mainImg.img = "";
        this.infoUa.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.infoUa.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.infoUa.mainImg.id) {
          this.infoUa.mainImg.id = new Date().valueOf();
        }
        this.infoUa.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.infoUa.mainImg.url = "";
      this.infoUa.mainImg.mainLocal = "";
      this.infoUa.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
