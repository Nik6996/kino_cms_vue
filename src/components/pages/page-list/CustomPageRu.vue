      ...
      <div class="card-page__content">
        <div class="card-page__header">
          <div class="card-page__name">
            <span>Название</span>
            <input v-model="customRu.name" placeholder="Название" type="text" />
          </div>
        </div>
        <div class="card-page__description">
          <span>Описание</span>
          <textarea
            v-model="customRu.description"
            ...
            <div class="card-page__seo-input">
              ...
              <p class="card-page__seo-description">
                Description:<textarea
                  v-model="customRu.seoDescription"
...
export default {
  ...
  computed: {
    imageSrc() {
      if (this.customRu.mainImg.localUrl) {
        return this.customRu.mainImg.localUrl;
      }
      if (this.customRu.mainImg.imgUrl) {
        return this.customRu.mainImg.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    previewMain() {
      if (!this.$refs.mainImg || !this.$refs.mainImg.files?.length) {
        this.customRu.mainImg.img = "";
        this.customRu.mainImg.localUrl = "";
        return;
      }
      const file = this.$refs.mainImg.files[0];
      const reader = new FileReader();
      this.customRu.mainImg.img = file;
      reader.onload = (ev) => {
        if (!this.customRu.mainImg.id) {
          this.customRu.mainImg.id = new Date().valueOf();
        }
        this.customRu.mainImg.localUrl = ev.currentTarget.result;
      };
    ...
    imgMainRemove() {
      this.mainImg.file = "";
      this.customRu.mainImg.url = "";
      this.customRu.mainImg.mainLocal = "";
      this.customRu.mainImg.img = "";
      this.mainImg.id = "";
    },
  },
