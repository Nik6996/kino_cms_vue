  ...
  <div>
    <div class="cinema">
      <div class="cinema__name">
        <p>Назва кiнотеатру:</p>
        <input
          v-model="cinemaUa.name"
          type="text"
          placeholder="Назва кiнотеатру"
        />
      </div>
      <div class="cinema__description">
        <p>Опис:</p>
        <textarea v-model="cinemaUa.description" placeholder="Текст" />
      </div>
      <div class="cinema__conditions">
        <p>Умови:</p>
        <textarea v-model="cinemaUa.conditions" placeholder="Текст" />
      </div>
      ...
      <div class="cinema__banner-img">
        <span>Фото верхнього банера</span>
      ...
      <div class="cinema__gallary gallary">
        ...
        <div class="gallary__main">
          <span class="gallary__size"> Розмiр: 1000x190</span>
      ...
      <div class="cinema__list-hall hall">
        ...
        <div class="hall__title">
          <div class="hall__name">Назва</div>
          <div class="hall__date">Дата створення</div>
        </div>
        ...
        <div class="hall__create">
          <button @click="saveLocal()">
            <router-link
              :to="{
                name: 'hallCard',
                params: { idCinema: this.cinemaUa.cinemaId },
              }"
              >Створити Зал</router-link
            >
          </button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    logoSrc() {
      if (this.cinemaUa.logo.logoLocal) {
        return this.cinemaUa.logo.logoLocal;
      }
      if (this.cinemaUa.logo.imgUrl) {
        return this.cinemaUa.logo.imgUrl;
      } else {
        return defaultImg;
      }
    },
    bannerSrc() {
      if (this.cinemaUa.banner.bannerLocal) {
        return this.cinemaUa.banner.bannerLocal;
      }
      if (this.cinemaUa.banner.imgUrl) {
        return this.cinemaUa.banner.imgUrl;
      } else {
        return defaultImg;
      }
    },
  ...
  methods: {
    ...
    bannerRemove() {
      ...
      this.cinemaUa.banner.imgUrl = "";
      this.bannerImg.bannerId = "";
      this.cinemaUa.banner.bannerLocal = "";
    },
    logoRemove() {
      this.logoImg.logoFile = "";
      this.cinemaUa.logo.imgUrl = "";
      this.logoImg.logoId = "";
      this.cinemaUa.logo.logoLocal = "";
    },
    previewLogo() {
      ...
      this.cinemaUa.logo.img = file;
      reader.onload = (ev) => {
        this.logoImg.logoFile = file;
        if (!this.cinemaUa.logo.id) {
          this.cinemaUa.logo.id = new Date().valueOf();
        }

        this.cinemaUa.logo.logoLocal = ev.currentTarget.result;
      };
    ...
    previewBanner() {
      ...
      const reader = new FileReader();
      this.cinemaUa.banner.img = file;
      reader.onload = (ev) => {
        this.bannerImg.bannerFile = file;
        if (!this.cinemaUa.banner.id) {
          this.cinemaUa.banner.id = new Date().valueOf();
        }
        this.cinemaUa.banner.bannerLocal = ev.currentTarget.result;
      };
      reader.readAsDataURL(file);
    },
